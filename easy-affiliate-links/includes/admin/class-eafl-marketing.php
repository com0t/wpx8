...
class EAFL_Marketing {
	...
	public static function page_template() {
		...
		if ( EAFL_Addons::is_active( 'premium' ) ) {
			...
			echo '<a href="https://bootstrapped.ventures/easy-affiliate-links/get-the-plugin/' . $params . '" target="_blank" class="button button-primary" style="font-size: 14px;">Get the Premium plugin now</a>';
		}
	...
	public static function marketing_notice( $notices ) {
		if ( ! EAFL_Addons::is_active( 'premium' ) ) {
			$notices[] = array(
				...
				'text' => '<a href="' . esc_url( admin_url( 'admin.php?page=eafl_marketing' ) ). '">' . self::$campaign['notice_text'] . '</a>',
			);
		}
