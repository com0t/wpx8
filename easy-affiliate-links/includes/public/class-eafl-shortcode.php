		...
		if ( $id ) {
			...
			if ( $link ) {
				...
				if ( 'html' === $link->type() ) {
					...
					if ( $html ) {
						$output = '<span data-eafl-id="' . $link->ID() . '" class="' . implode( ' ', $classes ) . '" style="display: inline-block;">' . $html . '</span>';
					...
					if ( '' === trim( $link->url() ) ) {
						...
						$output = '<a href="' . $url . '" data-eafl-id="' . $link->ID() . '" class="' . implode( ' ', $classes ) . '" target="' . $link->target() . '"' . $rel . '>' . $text . '</a>';
					}
